...
class BillDisplay extends Component {
	render() {
		...
		for (let i = 0; i < bill_items.length; i++) {
			bill +=
				(Number(bill_items[i][0][4]) +
					Number(bill_items[i][0][4]) * (Number(bill_items[i][0][6]) / 100)) *
				bill_items[i][1];
		}
				...
				<div className="card">
					<div className="card-body">
						...
						<p className="card-text">
							Amount in Rs : {parseFloat(bill).toFixed(2)}
						</p>
...
export default connect(mapStateToProps)(BillDisplay);
